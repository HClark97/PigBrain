...
for group in range (len(groupname)): #2: nox and nonNox
    for sets in range(len(data['NonnoxERP']['block'])): #48 sets 
        for channel in range(len(data['NonnoxERP']['block'][0]['channel'])): #32 channels
            for epoch in range(len(data['NonnoxERP']['block'][0]['channel'][0]['ERPs'][0])): #100 stimulations
                if np.size(data[groupname[group]]['block'][sets]['channel'][channel]['ERPs']): #If there are stimulations 
                    print('Im epoch no: '+ str(epoch) + ' channel: '+ str(channel) + ' set: '+ str(sets))
